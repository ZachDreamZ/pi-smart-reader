...
- **Critical: Parser initialization failure** — `wasm/tree-sitter.wasm` and `wasm/tree-sitter-typescript.wasm` were 9-byte placeholders, causing `Parser.init()` to fail silently. Replaced with real WASM binaries from tree-sitter.
- **Parser API mismatch** — `Parser.init({ wasmPath })` used deprecated API. Now uses `Parser.init({})` + `readFileSync` + `Parser.Language.load()` like pi-impact-analyzer.
- **`smartReader` reference runtime error** — `(smartReader as any).getStatus = getStatus` relied on function hoisting. Replaced with proper `updateSmartReaderState()` plus module-scoped closures.

### Changed

- **Parser.ts** — rewritten to use standard tree-sitter WASM loading pattern (matches pi-impact-analyzer's approach)
- **Index.ts** — `smartReader` named export now delegates through module-scoped state variables instead of setting function properties
